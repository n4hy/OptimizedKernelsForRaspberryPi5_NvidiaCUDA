...
void caf_f32(float* out_mag,
    ...
    for (std::size_t d = 0; d < n_doppler_bins; ++d) {
        ...
        // Build phase vector and compute cos/sin via fast NEON approximations
        for (std::size_t i = 0; i < n_samples; ++i) {
            phase_buf[i] = phase_step * i;
        }
        neon::neon_fast_cos_f32(cos_buf.data(), phase_buf.data(), n_samples);
        neon::neon_fast_sin_f32(sin_buf.data(), phase_buf.data(), n_samples);

        // Apply Doppler shift: shifted = ref * exp(j * phase)
        neon::neon_complex_mul_f32(shifted_re.data(), shifted_im.data(),
                                   ref_re, ref_im,
